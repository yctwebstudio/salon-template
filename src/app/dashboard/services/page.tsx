    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-xs text-[#1D1D1F]/40">管理沙龍提供的服務分類，定價請至各設計師的「定價矩陣」設定</p>
        </div>
      ...
      {/* 新增/編輯表單 */}
      {showForm && (
        <div className="bg-white border border-[#1D1D1F] p-6 mb-6 max-w-md">
          ...
          {services.map(s => (
            <div key={s.id} className="bg-white border border-[#D2D2D7] px-4 py-3 flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                {s.description && <span className="ml-2 text-xs text-[#1D1D1F]/40">{s.description}</span>}
              </div>
